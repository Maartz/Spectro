...
final class RepoTests: XCTestCase {
    ...
    func testUpdateQuery() async throws {
        ...
        debugPrint("Initial results: \(results)")
        XCTAssertEqual(results.count, 1)
        ...
        debugPrint("Query after update: \(query)")
        results = try await repo.all(query: query)
        debugPrint("Results after update: \(results)")
    ...
    func testDeleteQuery() async throws {
        ...
        try await repo.delete(
            from: "test_users", where: ["name": ("=", .string("Tyler Durden"))])
    ...
    func testRepoCount() async throws {
        var count = try await repo.count(
            from: "test_users", where: ["name": ("LIKE", .string("%Doe"))])
        ...
        count = try await repo.count(
            from: "test_users",
            where: ["email": ("LIKE", .string("%example.com"))])
